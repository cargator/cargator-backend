...
const calculateTotalDistance = (latLongArray: Array<any>) => {
  ...
  for (let i = 0; i < latLongArray.length - 1; i++) {
    const { latitude: lat1, longitude: lon1 } = latLongArray[i];
    const { latitude: lat2, longitude: lon2 } = latLongArray[i + 1];
...
const getDirectionsUsingOlaMap = async (location1: any, location2: any) => {
  try {
    ...
    const distance = resp.data?.routes[0]?.legs[0]?.distance || 0;

    return { distance: distance };
  } catch (error) {
    console.log('getDirectionsUsingOlaMap error:', error);
  }
};
